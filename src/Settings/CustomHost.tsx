...
const CustomHost = memo(() => {
  ...
  return <>
            ...
            {settings.custom_hosts.length > 0 ? settings.custom_hosts.map(host => <Space>
                            <Tag onRemove={() => {
        remove(host);
      }}>{host}</Tag>
                        </Space>) : <Callout intent="warning" title="No host detected!" icon="info-sign">
