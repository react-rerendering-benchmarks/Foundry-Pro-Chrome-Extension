...
const DevelopmentToken = memo(({
  toast
}: DevelopmentTokenProps) => {
  const {
    settings
  } = useStore();
  const copyDevelopmentToken = (event: any) => {
    ...
    fetch("/multipass/api/tokens", requestOptions).then(res => res.json()).then(result => {
  ...
  return <>
            {settings.token_manager.dev_token_state ? <MenuItem icon="bug" text="Copy development token" onClick={copyDevelopmentToken} /> : ""}
        </>;
});
